...
#'
#' @description This function uses the haplotypes in the gene columns, (PfCRT, PfMDR1, Kelch13, PfDHPS and PfDHFR ) and classifies them as
...
#'     \item \code{Artemisinin}: for Kelch13.
...
#' @param drug_column A character string representing the drug column to filter by (default is Null).
...
#' Chloroquine, Multidrug, Artemisinin, Sulfadoxine, and Pyrimethamine.
#'
#'
#' @export
#'
gene_classifier <- function(df, drug_column = NULL, save_output = FALSE) {
    ...
    input_df %>%
      dplyr::mutate(
        Chloroquine = sapply(.data[[gene_col]], function(x) {
          ...
          if (is.na(x) || all(hap_vec == "-")) {
            return("missing")
          } else if (last_element == "-" && any(head(hap_vec, -1) != "-")) {
            return("undetermined")
          } else if (grepl("^\\[.*\\]$", last_element)) {
            return("mixed_resistant")
          } else if (last_element == "T") {
            return("resistant")
          } else if (last_element == "K") {
            return("sensitive")
          } else {
  ...
  mdr1_classifier <- function(input_df, gene_col) {
    ...
    input_df %>%
      dplyr::mutate(
        Multidrug = sapply(.data[[gene_col]], function(x) {
          hap_vec <- split_haplotype(x)
          ref_matches <- sum(hap_vec == ref)
          dash_count <- sum(hap_vec == "-")

          if (is.na(x) || dash_count >= 2) {
            return("missing")
          } else if (ref_matches == length(ref) || (ref_matches >= 2 && any(hap_vec == "-"))) {
            return("sensitive")
          } else if (any(grepl("^\\[.*\\]$", hap_vec))) {
            return("mixed_resistant")
          } else if (any(hap_vec != ref) && dash_count <= 1) {
            return("resistant")
          } else {
            return("undetermined")
          }
  ...
  kelch_classifier <- function(input_df, gene_col) {
    input_df %>%
      dplyr::mutate(
        Artemisinin = sapply(.data[[gene_col]], function(x) {
          if (is.na(x) || x == "-") {
            return("missing")
          } else if (x == "WT") {
            return("sensitive")
          } else if (grepl("WT", x) && x != "WT") {
            return("mixed_resistant")
          } else {
            return("resistant")
          }
  ...
  sulfadoxine_classifier <- function(input_df, gene_col) {
    ...
    input_df %>%
      dplyr::mutate(
        Sulfadoxine = sapply(.data[[gene_col]], function(x) {
          hap_vec <- split_haplotype(x)
          ...
          if (is.na(x) || second_element == "-") {
            return("missing")
          } else if (grepl("^\\[.*\\]$", second_element)) {
            return("mixed_resistant")
          } else if (second_element == "G") {
            return("resistant")
          } else if (second_element == "A") {
            return("sensitive")
          } else {
            return("undetermined")
          }
    ...
    input_df %>%
      dplyr::mutate(
        Pyrimethamine = sapply(.data[[gene_col]], function(x) {
          ...
          if (is.na(x) || third_element == "-") {
            return("missing")
          } else if (grepl("^\\[.*\\]$", third_element)) {
            return("mixed_resistant")
          } else if (third_element == "N") {
            return("resistant")
          } else if (third_element == "S") {
            return("sensitive")
          } else {
            return("undetermined")
          }
  ...
  # Checkmate for valid drug column
  if (!is.null(drug_column)) {
    checkmate::assert_choice(drug_column, choices = c("Chloroquine", "Multidrug", "Artemisinin", "Sulfadoxine", "Pyrimethamine"))
  }
