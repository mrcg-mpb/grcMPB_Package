...
mutation_frequency <- function(df, gene, gene_col, save_output = TRUE, period_name = "Full", time = NULL, map_data,
...
#' @keywords internal
#'
create_m_plots <- function(df, gene, gene_col, save_output = TRUE, period_name = "Full", map_data,
                           label_size = 2.5, circle_num_size = 3.1, scale_circle_size = 10, include_mixed = FALSE, ...) {
  ...
  result_table1 <- data.frame(
    Mutations = gene_info[[gene]]$positions,
  ...
  result_table1_melted <- result_table1 %>%
    tidyr::pivot_longer(cols = c(Wild, Mutation, Mixed, Missing), names_to = "Type", values_to = "Value") %>%
    tidyr::separate(Value, into = c("Count", "Percentage"), sep = " ", remove = FALSE) %>%
    dplyr::mutate(Count = as.numeric(gsub("[^0-9]", "", Count)),
  ...
  # build the plot
  m_barchart <- ggplot(result_table1_melted, aes(x = reorder(Mutations, -Count), y = Percentage, fill = Type)) +
  ...
  counts_by_location <- lapply(locations, function(loc) {
    ...
    data.frame(Location = loc, Total_Samples = total_samples,
               t(mapply(function(count, total) paste0(count, " (", round((count / total) * 100, 1), "%)"),
  ...
  colnames(result_table2) <- c("Location", "Total", gene_info[[gene]]$positions)

  # Generate proportion maps for each mutation by location
  mutation_table <- result_table2 %>%
    dplyr::mutate(across(-c(Location, Total), ~ as.numeric(gsub(".*\\((\\d+(?:\\.\\d+)?)%\\).*", "\\1", .)))) %>%
  ...
  for (p_column in gene_info[[gene]]$positions) {
