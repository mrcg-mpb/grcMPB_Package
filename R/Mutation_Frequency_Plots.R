...
mutation_frequency <- function(df, gene_col, filter_gene_col = FALSE, save_output = TRUE, period_name = "Full", time = NULL, map_data, label_repel = 1.3,
  ...
  create_m_plots <- function(df_input, period_name_input = period_name) {
    if (filter_gene_col) {
      df_input <- df_input %>%
    ...
    m_barchart <- ggplot(result_table1_melted, aes(x = stats::reorder(Mutations, -Count), y = Percentage, fill = Type)) +
      ...
      geom_text(aes(y = Percentage + 0.5, label = paste0(Percentage, "%")),
        vjust = -0.4, position = position_dodge(width = 0.9), size = 2.8, fontface = "bold"
      ) +
      scale_y_continuous(
        name = "Percentages (%)",
      ...
      labs(x = "Mutations", title = paste0("Mutation Frequency", " (", period_name_input, ")")) +
    ...
    locations <- unique(df_input[["Location"]])
    ...
    counts_by_location <- lapply(locations, function(loc) {
      loc_hap_vecs <- hap_vecs[df_input[["Location"]] == loc]
    ...
    if (save_output) {
      save_path <- file.path(get("Output_Dir", envir = .GlobalEnv), "Mutation_Frequency", gene_col)
      dir.create(save_path, showWarnings = FALSE, recursive = TRUE)

      save_path2 <- file.path(save_path, "Percentage_Maps")
      dir.create(save_path2, showWarnings = FALSE, recursive = TRUE)

      writexl::write_xlsx(result_table1, file.path(save_path, paste0(gene_col, "_MutationFrequency_Table1", "_", period_name_input, ".xlsx")))
      writexl::write_xlsx(result_table2, file.path(save_path, paste0(gene_col, "_MutationFrequency_Table2", "_", period_name_input, ".xlsx")))

      ggsave(
        filename = paste0(gene_col, "_Mutation_BarChart_", period_name_input, ".jpeg"),
        plot = m_barchart,
        path = save_path, dpi = plot_dpi, width = 10, height = 7
      )

      for (plot_name in names(m_maps)) {
        ggsave(
          filename = paste0(gene_col, "_", plot_name, "_", period_name_input, ".jpeg"),
          plot = m_maps[[plot_name]],
    ...
    return(list(
      Mutation_Plots = list(BarChart = m_barchart, Mt_Maps = m_maps),
      Mutation_Freq_Tables =  list(result_table1, result_table2)
    ))
  }

  # Handle cases with and without time filtering
  if (is.null(time)) {
    return(create_m_plots(df))
  } else {
